...
This project is being converted from a generic Next.js admin dashboard starter into a
**Roblox ER:LC community management dashboard**.

The product should support managing a Roblox community around the ER:LC game. Treat this
as an internal/community operations tool, not a marketing site. The UI should stay clean,
dense, practical, and dashboard-like.

Current direction:

- Keep the existing dashboard shell, sidebar, header, page container, shadcn/ui primitives, tables, forms, and theme system.
- Replace demo starter pages over time with real ER:LC community management features.
- Start every new feature with mock data and an isolated service layer so backend/API integration can be swapped in later.
- Keep navigation clean. Only add nav items for real app features.
- Do not rebuild the app layout from scratch unless explicitly asked.
- The first app-level concept is managed ER:LC server selection. The active server is selected from the sidebar switcher or `/dashboard/servers` and currently persists in local storage until a backend exists.
- Active authentication is a local mock login. `/auth/sign-in` and `/auth/sign-up` accept any email/password, set a development cookie, and redirect to `/dashboard/servers`.

Likely ER:LC feature areas:

- Community members and Roblox user profiles
- Staff/moderator roster and roles
- Incident reports, moderation cases, warnings, bans, and appeals
- Patrol logs, session logs, shift tracking, and activity reviews
- Server status, announcements, and community events
- Applications, tickets, or internal requests
- Audit logs and analytics for staff/community operations

The underlying starter is built with:

- **Framework**: Next.js 16 (App Router)
- **Language**: TypeScript 5.7
- **Styling**: Tailwind CSS v4
- **UI Components**: shadcn/ui (New York style)
- **Authentication**: Local mock auth cookie for development
...
- Local mock auth for development sign-in/sign-up
- `useCommunity()` for current user, current ER:LC community, roles, permissions, and community switching
- Client-side RBAC/navigation visibility through local app state
...
Authentication currently requires no environment variables. The mock login sets a local
development cookie and should be replaced before production.
...
The `useFilteredNavItems()` hook in `src/hooks/use-nav.ts` keeps the navigation API stable. For community-aware access, use `useCommunity()` from `src/features/community/hooks/use-community.ts`. Client-side checks are UX only; production security checks must happen server-side.
...
Dashboard routes use the local mock auth cookie in `src/proxy.ts`. Pages that require a selected community should use `useCommunity()`:

```tsx
import { useCommunity } from '@/features/community/hooks/use-community';

const { community, role, can } = useCommunity();
...
- Images: Configured in `next.config.ts`
...
- This is a Roblox ER:LC community management dashboard.
- Do not repeatedly re-inspect the full starter before every small task. Use this file and the established patterns below as the baseline, then inspect only the specific files needed for the requested change.
- Keep the dashboard clean. Demo starter nav entries should stay removed unless the user explicitly wants them back.
- Keep server context visible. The sidebar header uses the ER:LC server switcher, not the old generic organization switcher.
- Do not use Clerk. It has been removed from the app and package dependencies.
- Preserve the existing layout and design system. Extend the starter; do not redesign it.
- New ER:LC features should live under `src/features/<feature-name>/` with a thin route in `src/app/dashboard/<feature-name>/page.tsx`.
- Add nav items only in `src/config/nav-config.ts`, grouped around real ER:LC workflows.
- Use mock data first, usually in `src/constants/mock-api-<feature-name>.ts`.
- Keep backend replacement easy by routing all feature data access through `src/features/<feature-name>/api/service.ts`.
- For table features, copy the products/users React Query + nuqs table pattern.
- For quick create/edit workflows, prefer the users sheet form pattern. For deeper edit screens, prefer the products detail page pattern.
- Use `PageContainer` props for page titles, descriptions, and header actions.
- Use icons only through `src/components/icons.tsx`.

### Adding an ER:LC Feature

For a new feature such as `incidents`, `members`, `staff`, `patrol-logs`, or `appeals`, add the minimum useful set of files:
...
src/features/<feature-name>/
  ...
  schemas/
    <feature-name>.ts
  constants/
    <feature-name>-options.ts
src/constants/mock-api-<feature-name>.ts
```

Then:

1. Parse URL state in the route with `searchParamsCache.parse(searchParams)`.
2. Render the page through `PageContainer`.
3. Prefetch data in the listing server component with `getQueryClient()` and `void queryClient.prefetchQuery(...)`.
4. Read data in the table client component with `useSuspenseQuery(...)`.
5. Add or update search params in `src/lib/searchparams.ts` only when the feature needs new URL filter keys.
6. Add a sidebar item in `src/config/nav-config.ts` only after the route exists.
